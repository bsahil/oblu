...
class DeviceClient(threading.Thread):
    client_socket = ''
    outfile = ''
    def __init__(self, name, ip, port, queue, stepFile):
...
port = 9876
device_client = DeviceClient(device_ip, device_ip, port, queue, 'step.txt')
device_client2 = DeviceClient(device_ip2, device_ip2, port, queue, 'step1.txt')
device_client.start()
