...
class DeviceClient2(threading.Thread):
    ...
    def write_file_hdr(self):
        if os.path.exists(LOG_FILE2) :
            created_time = datetime.datetime.fromtimestamp(os.path.getctime(LOG_FILE2))
            backup_file = LOG_FILE2 + created_time.strftime("_%d%b%Y_%H%M")
            try:
                os.rename(LOG_FILE2, backup_file)
            except WindowsError:
                os.remove(backup_file)
                os.rename(LOG_FILE2, backup_file)

        self.outfile = open(LOG_FILE2,"ab+")
