...
#   06-06-2018 |    1.0       |   Ajit Gupta                     | Modified for WiFi connectivity
#
#
#
'''
...
# com_port = 'BB:A0:50:61:53:E4'
# com_port = '00:A0:50:E4:91:98'
host = '192.168.1.5'#'192.168.43.201'                        # Write hare serial port on your device
port = 9876
...
def open_device(host, port):
    print "Opening Serial device", host
    # client_socket = socket.socket()  # instantiate
...
def read_device(length):
    buffer = ''
    # client_socket.flushInput()
    data_len=0
    while data_len < length:
        buffer += client_socket.recv(length - data_len)
        data_len+=len(buffer)
        if data_len < length :
            time.sleep(0.005)
    return buffer
...
def write_device(buffer):
    buffer = ("".join(str('{0:02x}'.format(e)) for e in buffer)).decode("hex")
    # if(len(buffer) < MAX_CMD_LENGTH):
    client_socket.send(buffer)
        # client_socket.flushOutput()
    # else:
...
#         gatt.sendline('mtu 512')
#         gatt.expect('MTU was exchanged successfully:')
#         # gatt.expect("\r\n", timeout=10)
...
#
#
# def read_device(gatt):  # to read coming data packets from com port,length = packet size
#     buffer = []
#     # buffer = device.read(length)
#     gatt.expect('Notification handle = 0x000e value:')
#     buffer = gatt.before
#     return buffer
...
#     # device.write(buffer)
#     # device.flushOutput()
#     cmd = getMacAddress(buffer)
#     device.sendline('char-write-cmd 0x0011 {0}'.format(cmd))
#     # print 'char-write-cmd 0x0011 {0}'.format(cmd)
#     device.expect('')
...
covarianceWeightingFile = './Calibration_MIMU4444BT/static_20150224_225453_1'
target_board = 'OBLU'  # MIMU22BT, MIMU3333, MIMU4444, MIMU4444BT, OBLU
...
# try:
#     com.sendline('char-write-req 0x000f 01')
#     com.expect('Characteristic value was written successfully')
# except pexpect.exceptions.TIMEOUT as e:
#     tkMessageBox.showerror("oops", "%s\nPlease restart the device and com port and try again")
#     sys.exit(1)
...
while True:
    ...
    counter = desired_nr_samples + 25
    while counter > 0:
       pkt=read_device(58)
       log_data.write(pkt.encode("hex"))
       counter -= 1

    #stop output and close file
    write_device([34, 0, 34])
    log_data.close()
