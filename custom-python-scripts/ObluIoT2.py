...
class DeviceClient(threading.Thread):
    client_socket = ''
    outfile = ''
    def __init__(self, name, ip, port, queue, stepFile, oblu):
    ...
    def write_file_hdr(self):
        if os.path.exists(self.stepFile) :
            created_time = datetime.datetime.fromtimestamp(os.path.getctime(self.stepFile))
            backup_file = self.stepFile + created_time.strftime("_%d%b%Y_%H%M")
            try:
                os.rename(self.stepFile, backup_file)
            except WindowsError:
                os.remove(backup_file)
                os.rename(self.stepFile, backup_file)

        self.outfile = open(self.stepFile,"ab+")
...
port = 9876
device_client = DeviceClient(device_ip, device_ip, port, queue, 'step.txt',1)
device_client2 = DeviceClient(device_ip2, device_ip2, port, queue, 'step1.txt',2)
device_client.start()
